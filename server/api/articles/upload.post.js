import { db_insert, db_update, db_find } from "~/server/lib/db";
import { ai_generate } from '~/server/lib/ai';
...
export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        // 检查内容合规性
        let complianceResult = await checkContentCompliance(ContentText, uid);
        ...
        // 获取内容摘要
        let summaryResult = await generateSummary(ContentText, uid);
        if (!summaryResult) {
        ...
        // 获取新的slug值
        const { total } = await $fetch('/api/articles/get/list?size=0');
        const newSlug = parseInt(total) + 1;
...
// 内容合规性检查函数
async function checkContentCompliance(content, uid) {
    try {
...
// 生成摘要函数
async function generateSummary(content, uid) {
    try {
