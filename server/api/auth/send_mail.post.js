import { db_find, db_update, db_insert } from "~/server/lib/db";
import { sendCode } from "~/server/lib/mail";
...
async function sendCaptcha(email, code, requestType, registerEntry) {
    try {
        sendCode(email, code, requestType);
    } catch (e) {
        return { code: "500", message: e.toString(), status: "failed" };
    }
